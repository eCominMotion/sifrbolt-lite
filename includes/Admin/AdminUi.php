...
/**
 * Renders the administration experience for SifrBolt Spark.
 */
final class AdminUi {

	/**
	 * Sets up dependencies for the UI layer.
	 *
	 * @param AutoloadInspectorReader $autoload_reader Autoload analytics collector.
	 * @param AutoloadInspectorWriter $autoload_writer Autoload modifier.
	 * @param TransientsJanitor       $transients_janitor Transient cleanup manager.
	...
	// phpcs:disable Squiz.Commenting.VariableComment.Missing -- Typed private members self-document usage.
	...
	public function register_menu(): void {
		add_menu_page(
			...
			array( $this, 'render_command_deck' ),
			'dashicons-airplane'
		);
		...
		add_submenu_page( 'sifrbolt-command-deck', __( 'Flight Recorder', 'sifrbolt' ), __( 'Flight Recorder', 'sifrbolt' ), 'manage_options', 'sifrbolt-flight-recorder', array( $this, 'render_flight_recorder' ) );
	}
	...
	/**
	 * Outputs the Command Deck screen.
	 *
	 * @return void
	 */
	public function render_command_deck(): void {
		...
		<div class="wrap sifrbolt-command-deck">
			...
			<?php /* translators: %s: Plugin version string. */ ?>
	...
	/**
	 * Outputs the Citadel Wall screen.
	 *
	 * @return void
	 */
	public function render_citadel_wall(): void {
		...
		<div class="wrap">
			...
			<?php /* translators: %s: Autoload footprint size in kilobytes. */ ?>
						...
						<td>
							<?php if ( $write_enabled ) : ?>
								<form method="post" action="">
									...
									<input type="hidden" name="set_autoload" value="<?php echo 'yes' === $row['autoload'] ? 'no' : 'yes'; ?>" />
									<input type="submit" class="button" value="<?php echo 'yes' === $row['autoload'] ? esc_attr__( 'Set to no', 'sifrbolt' ) : esc_attr__( 'Set to yes', 'sifrbolt' ); ?>" />
								</form>
							<?php else : ?>
	...
	/**
	 * Outputs the Flight Recorder screen.
	 *
	 * @return void
	 */
